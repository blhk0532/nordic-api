        ...
        init() {
            this.calendar = new Calendar(this.$el, {
                ...
                eventClick: ({ event, jsEvent }) => {
                    ...
                    // Debug: log event click to help trace client-side invocation
                    console.debug('[filament-fullcalendar] eventClick', { id: event.id, title: event.title, extendedProps: event.extendedProps })
                ...
                dateClick: async ({ dateStr, allDay, view, resource }) => {
                    if (!selectable) return

                    console.debug('[filament-fullcalendar] dateClick', { date: dateStr, allDay, view, resource })

                    await this.$wire.onDateSelect(
                        dateStr,
                        null,
                        allDay,
                        view,
                ...
                select: async ({ startStr, endStr, allDay, view, resource }) => {
                    if (!selectable) return

                    console.debug('[filament-fullcalendar] select', { start: startStr, end: endStr, allDay, view, resource })
