...
class Color
{
    ...
    {
        $color1 = str_replace(' ', '', self::convertToRgb($color1));
        $color2 = str_replace(' ', '', self::convertToRgb($color2));
    ...
    {
        return self::calculateContrastRatio($color1, $color2) >= 4.5;
    }

    public static function isNonTextContrastRatioAccessible(string $color1, string $color2): bool
    {
        return self::calculateContrastRatio($color1, $color2) >= 3;
    }
    ...
    {
        $color = str_replace(' ', '', self::convertToRgb($color));
        ...
        foreach ($intensityMap as $shade => $intensity) {
            ...
            $colors[$shade] = self::convertToOklch(sprintf('%s, %s, %s', round($red), round($green), round($blue)));
        }
    ...
    {
        return self::generatePalette($color);
    }
    ...
    {
        $color = self::convertToOklch($color);
