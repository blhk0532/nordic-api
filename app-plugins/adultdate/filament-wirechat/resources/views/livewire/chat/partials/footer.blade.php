@php
    // Enable emoji picker and heart icon to match preview
    $hasEmojiPicker = true;
    $floatingEmojiPicker = true;
@endphp
        ...
        <div id="chat-footer" x-data="{ 'openEmojiPicker': false }"
            ...
            <section
                ...
                <section x-show="$wire.media.length>0 ||$wire.files.length>0" x-cloak
                    ...
                    @if (count($media) > 0)
                        <div x-data="attachments('media')">
                            ...
                            {{--
                                @error('media')
                            <span class="flex text-sm text-red-500 dark:text-red-400 pb-2 bg-[var(--wc-light-secondary)] dark:bg-[var(--wc-dark-secondary)] p-2 w-full justify-between">
                                    {{$message}}
                                ...
                                @foreach ($media as $key => $mediaItem)
                                    ...
                                    @if (str()->startsWith($mediaItem->getMimeType(), 'video/'))
                                        <div class="relative h-24 sm:h-36 ">
                                            ...
                                            <x-filament-wirechat::video height="h-24 sm:h-36 " :cover="false"
                                ...
                                <label wire:loading.class="cursor-progress"
                                    ...
                                    <input wire:loading.attr="disabled"
                                        @change="handleFileSelect(event,{{ count($media) }})" type="file" multiple
                                           accept="{{ collect($this->getMediaMimes())->map(fn($ext) => '.' . $ext)->implode(',') }}"
                                           class="sr-only">
                                    <span class="m-auto ">
                            ...
                            <label wire:loading.class="cursor-progress"
                                ...
                                <input wire:loading.attr="disabled"
                                    @change="handleFileSelect(event,{{ count($files) }})" type="file" multiple
                                       accept="{{ collect($this->getFileMimes())->map(fn($ext) => '.' . $ext)->implode(',') }}"
                ...
                @if ($replyMessage != null)
                    <section class="p-px py-1 w-full col-span-12">
                        <div class="flex justify-between items-center dark:text-white">
                            <h6 class="text-sm">
                                    {{ $replyMessage?->ownedBy($this->auth) ? __('filament-wirechat::chat.labels.replying_to_yourself'): __('filament-wirechat::chat.labels.replying_to',['participant'=>$replyMessage->sendable?->name])  }}
                            </h6>
                ...
                @endif
                    ...
                    @if (count($this->media) == 0 && count($this->files) == 0 && true)
                        <x-filament-wirechat::popover position="top" popoverOffset="70">
                                    ...
                                    <label wire:loading.class="cursor-progress" x-data="attachments('files')"
                                        ...
                                        <div
                                            ...
                                            <span class=" dark:text-white">
                                               @lang('filament-wirechat::chat.actions.upload_file.label')
                                            </span>
                                        </div>
                                    </label>
                                @endif
                                ...
                                @if (true)
                                    <label wire:loading.class="cursor-progress" x-data="attachments('media')"
                                        ...
                                        <div
                                            ...
                                            <span class=" dark:text-white">
                                               @lang('filament-wirechat::chat.actions.upload_media.label')
                                            </span>
                                        </div>
                                    </label>
                                @endif
                    ...
                    <div @class(['flex gap-2 sm:px-2 w-full'])>
                        <textarea @focus-input-field.window="$el.focus()" autocomplete="off" x-model='body' x-ref="body"
                            ...
                            type="text" name="message" placeholder="{{ __('filament-wirechat::chat.inputs.message.placeholder') }}" maxlength="1700" rows="1"
            ...
            @script
                <script>
                    Alpine.data('attachments', (type = "media") => ({
                        ...
                        MAXFILES: @json($this->getMaxUploads() ?? 10), // Maximum number of files allowed
                        maxSize:  @json(($this->getMediaMaxUploadSize() ?? 5) * 1024), // Max size per file (in bytes)
                        allowedFileTypes: type === 'media' ? @json($this->getMediaMimes()) :@json($this->getFileMimes()), // Allowed MIME types based on type
                        ...
                        validateFiles(files, count) {
                            ...
                            if (totalFiles > this.MAXFILES) {
                                ...
                                $dispatch('wirechat-toast', {
                                    type: 'warning',
                                    message: @js(__('filament-wirechat::validation.max.array', ['attribute' => __('filament-wirechat::chat.inputs.media.label'),'max'=>($this->getMaxUploads() ?? 10)]))
                                });
                            }
                                    ...
                                    if (file.size > this.maxSize) {
                                        $dispatch('wirechat-toast', {
                                            ...
                                            message:this.type==='media'?
                                                    @js(__('filament-wirechat::validation.max.file', ['attribute' => __('filament-wirechat::chat.inputs.media.label'),'max'=>($this->getMediaMaxUploadSize() ?? 5)])):
                                                    @js(__('filament-wirechat::validation.max.file', ['attribute' => __('filament-wirechat::chat.inputs.media.label'),'max'=>($this->getFileMaxUploadSize() ?? 10)]))
                                        ...
                                        $dispatch('wirechat-toast', {
                                            type: 'warning',
                                            message: this.type==='media'?
                                                    @js(__('filament-wirechat::validation.mimes', [ 'attribute' => __('filament-wirechat::chat.inputs.media.label'), 'values' => implode(', ', $this->getMediaMimes()) ])):
                                                    @js(__('filament-wirechat::validation.mimes', [ 'attribute' => __('filament-wirechat::chat.inputs.media.label'), 'values' => implode(', ', $this->getFileMimes()) ]))
