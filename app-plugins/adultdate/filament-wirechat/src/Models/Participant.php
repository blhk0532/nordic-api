...
class Participant extends Model
{
    ...
    public function removeByAdmin(Model|Authenticatable $admin): void
    {
        ...
        $exists = Action::where('actionable_id', $this->id)
            ->where('actionable_type', self::class)
    ...
    {
        self::addGlobalScope('withoutExited', function ($query) {
            $query->whereNull('exited_at');
        });

        self::addGlobalScope(WithoutRemovedActionScope::class);

        // listen to deleted
        self::deleted(function ($participant) {
