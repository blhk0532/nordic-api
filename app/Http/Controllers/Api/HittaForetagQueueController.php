...
class HittaForetagQueueController extends Controller
{
    ...
    {
        $record = HittaForetagQueue::findOrFail($id);
    ...
    {
        $record = HittaForetagQueue::query()
    ...
    public function bulkUpdate(Request $request): JsonResponse
    {
        ...
        foreach ($validated['records'] as $index => $recordData) {
            try {
                ...
                $record = HittaForetagQueue::where('post_nummer', $normalizedPostNummer)->first();
    ...
    public function updateByPostNummer(Request $request, string $postNummer): JsonResponse
    {
        ...
        $record = HittaForetagQueue::where('post_nummer', $normalizedPostNummer)->firstOrFail();

        $record->update($validated);

        return response()->json([
            'message' => 'HittaForetagQueue record updated successfully',
            'data' => $record,
        ]);
    }
}
