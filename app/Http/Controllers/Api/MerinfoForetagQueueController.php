...
class MerinfoForetagQueueController extends Controller
{
    public function index(Request $request): JsonResponse
    {
        $query = MerinfoForetagQueue::query();
    ...
    {
        $record = MerinfoForetagQueue::findOrFail($id);
    ...
    {
        $record = MerinfoForetagQueue::query()
    ...
    public function bulkUpdate(Request $request): JsonResponse
    {
        ...
        foreach ($validated['records'] as $index => $recordData) {
            try {
                ...
                if (mb_strlen($digitsOnly) >= 5) {
                    $normalizedPostNummer = mb_substr($digitsOnly, 0, 3).' '.mb_substr($digitsOnly, 3, 2);
                } else {
                    $normalizedPostNummer = $digitsOnly;
                }

                $record = MerinfoForetagQueue::where('post_nummer', $normalizedPostNummer)->first();
    ...
    public function updateByPostNummer(Request $request, string $postNummer): JsonResponse
    {
        ...
        $record = MerinfoForetagQueue::where('post_nummer', $normalizedPostNummer)->firstOrFail();

        $record->update($validated);

        return response()->json([
            'message' => 'MerinfoForetagQueue record updated successfully',
            'data' => $record,
        ]);
    }
}
