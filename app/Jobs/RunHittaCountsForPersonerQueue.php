...
class RunHittaCountsForPersonerQueue implements ShouldQueue
{
    ...
    public int $timeout = 6000000; // 100 minutes
    ...
    public function handle(): void
    {
        ...
        Log::info("[Hitta Counts {$record->post_nummer}] Starting count extraction");
        ...
        $query = mb_trim($record->post_nummer);
        ...
        if (! $process->successful()) {
            Log::error("[Hitta Counts {$record->post_nummer}] Failed: ".$process->errorOutput());
            $record->update([
        ...
        if ($counts === null) {
            Log::error("[Hitta Counts {$record->post_nummer}] Failed to extract counts from output: ".$process->output());
            $record->update([
                'personer_status' => 'failed',
                'is_active' => false,
            ]);
        ...
        Log::info("[Hitta Counts {$record->post_nummer}] Extracted counts: ".json_encode($counts));
        ...
        Log::info("[Hitta Counts {$record->post_nummer}] Count extraction completed");
    }
