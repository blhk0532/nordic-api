...
class HittaScraper {
  constructor(api_url, api_token) {
    this.api_url = ('https://ekoll.se' || process.env.API_URL || process.env.APP_URL || '').replace(/\/+$/, '');
  ...
  getDbConnection() {
    /** Get or create PostgreSQL database pool connection */
    if (this.db) {
      return this.db;
    }

    // Create PostgreSQL pool
    this.db = new pg.Pool(this.dbConfig);

    console.log(`✓ Connected to PostgreSQL database: ${this.dbConfig.database}@${this.dbConfig.host}:${this.dbConfig.port}`);
    return this.db;
  }

  isValidRecord(record, targetPostnummer = null) {
    /**
     * Validate that a record has all required fields before saving
     * Required: gatuadress, postort, postnummer, personnamn
     * If targetPostnummer provided, postnummer must match
     */
    const hasRequired = record.personnamn && record.gatuadress && record.postort && record.postnummer;
    ...
    if (targetPostnummer) {
      ...
      const targetNormalized = targetPostnummer.replace(/\s+/g, '').toUpperCase();
  ...
  async saveHittaToDatabase(hittaData, targetPostnummer = null) {
    /**
     * Save Hitta data via API
     * Only saves if record has all required fields: personnamn, gatuadress, postort, postnummer
     * If targetPostnummer provided, postnummer must match
     */
    // Validate required fields
    if (!this.isValidRecord(hittaData, targetPostnummer)) {
      const reason = !hittaData.personnamn ? 'missing personnamn'
        : !hittaData.gatuadress ? 'missing gatuadress'
        : !hittaData.postort ? 'missing postort'
        : !hittaData.postnummer ? 'missing postnummer'
        : targetPostnummer ? `postnummer mismatch (expected ${targetPostnummer}, got ${hittaData.postnummer})`
        : 'unknown validation error';

      console.log(`  ⊘ Skipping ${hittaData.personnamn || 'Unknown'}: ${reason}`);
      return false;
    }
    ...
    try {
      ...
      if (hittaData.gatuadress && isHusFalsePattern.test(hittaData.gatuadress)) {
        apiData.is_hus = false;
      }
  ...
  closeDbConnection() {
    /** Close PostgreSQL database pool connection */
    if (this.db) {
      this.db.end().catch(() => {});
      this.db = null;
    }
  ...
  async saveSwedenPerson(personData, postnummer, targetPostnummer = null) {
    // Validate required fields
    if (!this.isValidRecord(personData, targetPostnummer || postnummer)) {
      const reason = !personData.personnamn ? 'missing personnamn'
        : !personData.gatuadress ? 'missing gatuadress'
        : !personData.postort ? 'missing postort'
        : !personData.postnummer ? 'missing postnummer'
        : targetPostnummer ? `postnummer mismatch (expected ${targetPostnummer}, got ${personData.postnummer})`
        : 'unknown validation error';

      console.log(`  ⊘ Skipping sweden_personer for ${personData.personnamn || 'Unknown'}: ${reason}`);
      return false;
    }
    ...
    try {
      ...
      // Try to split personnamn into fornamn and efternamn
      if (personData.personnamn) {
        const parts = personData.personnamn.trim().split(/\s+/);
        if (parts.length > 1) {
          fornamn = parts[0];
          efternamn = parts.slice(1).join(' ');
        } else {
          fornamn = personData.personnamn;
        }
      }
  ...
  /**
   * Update postnummer queue count after processing
   */
  ...
  async saveToDatabase(records = this.results, targetPostnummer = null) {
    /**
     * Save Hitta search results to hitta_data table and optionally sweden_personer
     * Only saves records with all required fields: personnamn, gatuadress, postort, postnummer
     * If targetPostnummer provided, postnummer must match
     */
    ...
    let savedCount = 0;
    let swedenPersonCount = 0;

    for (const record of records) {
      try {
        // Save to hitta_data table via API
        const success = await this.saveHittaToDatabase(record, targetPostnummer);
        if (success) {
          savedCount++;

          // Also save to sweden_personer if we have the postnummer
          if (record.postnummer) {
            const personSaved = await this.saveSwedenPerson(record, record.postnummer, targetPostnummer);
            if (personSaved) {
              swedenPersonCount++;
            }
          }
    ...
    if (swedenPersonCount > 0) {
      console.log(`✓ Saved ${swedenPersonCount}/${records.length} records to sweden_personer table`);
    }
    return savedCount;
  }
...
async function main() {
  ...
  try {
    ...
    if (options.kommun) {
      ...
      console.log(`\n✓ Completed processing for kommun: ${options.kommun}`);
