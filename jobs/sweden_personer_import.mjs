...
async function processPersonRow(scraper, row, connection, apiOnly = false) {
	...
	try {
		...
		// If ratsit_link is missing, fetch it from search page
		let ratsitLink = row.ratsit_link;
		if (!ratsitLink) {
			const searchQuery = [row.fornamn, row.efternamn].filter(Boolean).join(' ') + (row.adress ? ', ' + row.adress : '');
			const searchUrl = `https://www.ratsit.se/sok/person?vem=${encodeURIComponent(searchQuery)}`;
			await page.goto(searchUrl, { waitUntil: 'domcontentloaded', timeout: 60000 });
			await page.waitForTimeout(1500);
			// Find first a href in .search-result-list ul
			ratsitLink = await page.evaluate(() => {
				const ul = document.querySelector('ul.search-result-list');
				if (!ul) return null;
				const a = ul.querySelector('a[href]');
				if (!a) return null;
		...
		const scraped = await scraper.scrapePersonPage(page, ratsitLink);
