...
<ul wire:loading.delay.long.remove wire:target="search" class="p-2 grid w-full space-y-2">
    ...
    id="conversation-{{ $conversation->id }}"
        ...
        @endif
            ...
            <div class="shrink-0">
                <x-filament-wirechat::avatar key="chat-list-conversation-{{$key}}" wire:key="chatslist-key-{{$key}}" disappearing="{{ $conversation->hasDisappearingTurnedOn() }}"
                ...
                <div
                    ...
                    <div class="flex gap-1 mb-1 w-full items-center">
                        ...
                        @if ($conversation->isSelfConversation())
                            <span class="font-medium dark:text-white">({{__('filament-wirechat::chats.labels.you')  }})</span>
                        @endif
                    ...
                    {{-- Message body --}}
                    @if ($lastMessage != null)
                        @include('filament-wirechat::livewire.chats.partials.message-body')
                    @endif
