        ...
        <div class="flex gap-x-3 items-center  ">
            ...
            <button
                ...
                <svg class="w-8 h-8 -mb-1 text-gray-500 cursor-pointer hover:text-gray-900 dark:hover:text-gray-200 dark:text-gray-300"
                    xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
            ...
            <x-filament-wirechat::actions.new-chat widget="{{$this->isWidget()}}" panel="{{$this->panelId()}}" >
            ...
            </x-filament-wirechat::actions.new-chat>
            @endif


            {{-- Only show if is not widget --}}
            @if ($redirectToHomeAction)
            <a id="redirect-button" href="{{ $this->dashboardRoute() }}" class="flex items-center">
        ...
        <section class="mt-4">
            <div class="max-w-[320px] px-2 rounded-lg dark:bg-[var(--wc-dark-secondary)]  bg-[var(--wc-light-secondary)] py-2 grid grid-cols-12 items-center">
                ...
                <input id="chats-search-field" name="chats_search" maxlength="100" type="search" wire:model.live.debounce='search'
                    placeholder="{{ __('filament-wirechat::chats.inputs.search.placeholder')  }}" autocomplete="off"
                    class="wc-input col-span-11 border-0 h-9 px-3 bg-inherit dark:text-white outline-hidden w-full focus:outline-hidden  focus:ring-0 hover:ring-0">
