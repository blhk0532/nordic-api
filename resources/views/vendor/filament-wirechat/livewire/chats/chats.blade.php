...
<div
    ...
    {{-- include header --}}
    @includeWhen($showHeader, 'filament-wirechat::livewire.chats.partials.header')
    ...
    <main x-data
        ...
        @if (count($conversations) > 0)
            {{-- include list item --}}
            @include('filament-wirechat::livewire.chats.partials.list')


            {{-- include load more if true --}}
            @includeWhen($canLoadMore, 'filament-wirechat::livewire.chats.partials.load-more-button')
        @else
            <div class="w-full flex items-center h-full justify-center">
                <h6 class=" font-bold text-gray-700 dark:text-white">{{ __('filament-wirechat::chats.labels.no_conversations_yet')  }}</h6>
            </div>
        @endif
    </main>
