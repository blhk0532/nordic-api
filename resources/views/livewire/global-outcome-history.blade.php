<div
    id="global-outcome-history"
    x-data="{ open: false }"
    x-show="open"
    x-on:open-modal.window="if ($event.detail.id === 'global-outcome-history') { open = true }"
    x-on:close-modal.window="if ($event.detail.id === 'global-outcome-history') { open = false }"
    style="display: none;"
...
>
    ...
    >
        <div class="h-full outcome-history-modal-widget" style="min-height: 100%; overflow: hidden;">
            <div class="flex flex-col h-full">
                ...
                <div class="flex-1 overflow-hidden" style="height: calc(100vh - 80px);">
                    @livewire(\App\Filament\App\Resources\RingaData\Widgets\OutcomeHistoryWidget::class)
                </div>
            </div>
        </div>
    </div>
